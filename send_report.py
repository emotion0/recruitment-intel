...
def generate_ai_insight(report_content: str) -> str:
    ...
    prompt = f"""你是大学生求职领域的资深分析师。以下是近3天内大学生求职平台的最新动态（均来自公开搜索结果原文，未经加工）。请用中文写一段200-350字的分析，分3-4条要点：

1. 提炼最值得关注的1-2条核心动态（涉及大学生付费服务、平台数据的优先）
2. 分析面向大学生的付费相关行为趋势（付费内推、简历优化、会员付费等）
3. 指出值得关注的舆论或争议点（学生投诉、骗局曝光等）
4. 如果各招聘平台有针对大学生群体的新动作，点出来

用专业、简练的语气，每条要点前加 🔥 或 📊 或 ⚠️ 或 🤝 等emoji。只基于给出的动态进行分析，不要编造任何信息。如果信息不足，就如实说"近3日信息量较少"。不需要开头寒暄，直接进入内容。
...
def main():
    ...
    subject = f"【大学生求职竞品付费情报】{report.stem}"
    if dup_rate > 0.8:
        subject += "（增量更新）"
